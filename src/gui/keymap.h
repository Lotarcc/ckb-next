...
public:
    ...
    inline static bool  isMouse(Model model)        { return model == M65 || model == SABRE || model == SCIMITAR || model == HARPOON || model == GLAIVE || model == KATAR || model == M65E || model == M95 || model == IRONCLAW || model == NIGHTSWORD || model == M55; }
